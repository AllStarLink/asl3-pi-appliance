...
const setbutton = document.getElementByIf("set-wifi");
...
function () set_wifi_run {
    /* global cockpit */
	result.innerHTML = `<span class="wifi-scan-loader"></span>`;
	const net_name = document.getElementByIf("wifi-ssid").value;
	const net_key = document.getElementByIf("wifi-key").value;
...
function set_wifi_fail() {
	result.innerHTML += `<span class="wifi-scan-error">WiFi set failed</scan>`;
}
